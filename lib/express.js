...
module.exports = function initialize (adapter, opts) {
  ...
  let searchRange = (fn, {scId, minHeight}, cb) => {
    ...
    try {
      adapter.tips((err, data) => {
        ...
        let results = []
        const getChunk = (hs) => {
          let mh = Math.max(hs - chunkSize, minHeight)
          ...
          adapter[fn]({scId, heightRange: [mh, hs], mempool: true}, maxResults, (err, ndata) => {
            ...
            if (mh > minHeight && results.length < resultSizeMax) {
              setImmediate(() => {
                try {
                  debug("Getting chunk from mh: " + (mh - 1).toString())
                  getChunk(mh - 1)
                } catch (e) {
        ...
        try {
          debug('scriptrange search:', data.txo.height, data)
          getChunk(data.txo.height)
        } catch (e) {
          debug('searchRange, after tips!', e)
          cb(e, null)
        }
      })
    } catch (e) {
      debug('searchRange, before tips!', e)
      cb(e, null)
    }
  }
  ...
  router.get('/status', (req, res) => {
    parallel({
      ...
      // debug("result is: " + JSON.stringify(results));
      if (err) return respond(req, res, err)
      // if (!results.localtip.txo) results.localtip.txo = { height : 0};
  ...
  function addressToScriptId(address) {
    ...
    debug("addressToScriptId " + bitcoin.crypto.sha256(script).toString('hex'))
  ...
  router.get('/a/:address/utxos', (req, res) => {
    let scId
    try {
      debug('utxos:', req.params.address)
    ...
    parallel({
      ...
      debug("Respond utxos. Finished request")
...
let inspect = require('util').inspect
  ...
  router.get('/a/:address/txs', (req, res) => {
    ...
    // let minHeight = resolveHeight(req.query.min_height)
    let minHeight = Number(req.query.min_height) || 1; 
    let verbose = req.query.verbose ? true : false

    searchRange('transactionIdsByScriptRange', {scId, minHeight}, (err, txIdSet) => {
      if (err) return respond(req, res, err)
  ...
  router.get('/tx/:txhex', (req, res) => {
    const txhex = req.params.txhex;
    rpc('getrawtransaction', [txhex, true], function (err, result) {
      if (err) {
...
//DEBUG=index,zmq,zmq:block,express,index:rpc,service,indexd